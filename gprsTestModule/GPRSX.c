...
USHORT gprsOpen()
{
	...
	CTOS_LCDTClearDisplay();
	sprintf(str,"%d",gprsConfig.iSocket);
	CTOS_LCDTPrint(str);
	CTOS_LCDTPrint("\n");
	CTOS_LCDTPrint(gprsConfig.baLocalIP);
	CTOS_LCDTPrint("\n");
	CTOS_LCDTPrint(gprsConfig.APN);
	CTOS_LCDTPrint("\n");
	CTOS_LCDTPrint(gprsConfig.baUrl);
	CTOS_KBDGet(&key2);
	...
	ret = CTOS_TCP_GPRSOpen("\x00\x00\x00\x00","internet.vodafone.net" ,"" ,"");
#ifdef DEBUG_LCD
